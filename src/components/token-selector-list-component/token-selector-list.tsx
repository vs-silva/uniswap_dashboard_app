...
export function TokenSelectorList(props: { tokens: CryptoTokenDTO[]; }): JSX.Element {
    ...
    return (
        <div className="flex justify-center" data-testid="token-selector-list-container">
            <ul className="bg-white border border-gray-200 w-full text-gray-900" data-testid="token-selector-list">
                {
                    tokens.map((token, index) => (
                        <li data-testid={`token-selector-list-item-${index}`} className="px-6 py-2 border-b border-gray-200 w-full cursor-pointer" key={index}
                        onClick={(event) => {
